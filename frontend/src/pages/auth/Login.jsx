    ...
    <div style={{
      ...
      <div className="hidden lg:flex flex-col justify-between p-14"
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '.72rem', color: ORANGE, fontWeight: 700 }}>Portfolio demo</p>
          </div>
        </div>
        ...
        <p style={{ position: 'relative', fontSize: '.78rem', color: 'rgba(255,255,255,.2)' }}>
          © {new Date().getFullYear()} SolarJi — Portfolio demo
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: 400 }}>
          ...
          <div style={{
            marginTop: '1.75rem', padding: '1rem 1.1rem', borderRadius: 12,
            background: '#fff8f0', border: `1px solid rgba(247,148,29,.25)`,
          }}>
            <p style={{ margin: '0 0 4px', fontSize: '.78rem', fontWeight: 800, color: '#9a3412' }}>
              Portfolio demo
            </p>
            <p style={{ margin: '0 0 12px', fontSize: '.72rem', color: '#b45309', lineHeight: 1.45 }}>
              Click a role to fill credentials. Password for all: <strong>Demo@123</strong>
            </p>
              ...
              {DEMO_ACCOUNTS.map((account) => (
                <button
                  ...
                  style={{
                    ...
                    border: form.email === account.email ? `1.5px solid ${ORANGE}` : '1px solid rgba(247,148,29,.2)',
                    background: form.email === account.email ? 'rgba(247,148,29,.12)' : '#fff',
                    textAlign: 'left',
                  }}
                >
                  <span>
                    <span style={{ display: 'block', fontSize: '.8rem', fontWeight: 700, color: '#111' }}>{account.label}</span>
                    <span style={{ display: 'block', fontSize: '.68rem', color: '#9ca3af' }}>{account.description}</span>
                  </span>
                  <span style={{ fontSize: '.65rem', color: '#b45309', fontFamily: 'monospace', flexShrink: 0 }}>
