...
import { DEMO_COMPANY } from '../../constants/demoCompany';
...
const SERVICES = [
  ...
  { icon:Award,  title:'Net Metering',       desc:'End-to-end utility documentation, meter installation & grid synchronisation.',  photo: PHOTOS.panels  },
];

const PRODUCTS = [
  { name:'Solar Panels',       brands:'Brand A · Brand B · Brand C', desc:'Mono PERC & Bifacial 400–550W' },
  { name:'Inverters',          brands:'Brand X · Brand Y',           desc:'String & Hybrid 1kW–100kW'    },
  { name:'Batteries',          brands:'Sample Battery Lines',        desc:'Lead Acid & Li-ion 100–200Ah'  },
  { name:'Mounting Structure', brands:'GI & Aluminium alloy',        desc:'RCC, tin roof & ground mount'  },
  { name:'DC / AC Cable',      brands:'Solar-grade cable',           desc:'4 / 6 / 10 mm² solar grade'   },
  { name:'ACDB / DCDB',        brands:'Custom panels',               desc:'Protection & distribution box' },
];

const STATS = [
  { value:'120+',  label:'Sample Installs'    },
  { value:'0.5 MW', label:'Demo Capacity'     },
  { value:'250+',  label:'Demo Customers'     },
  { value:'Demo',  label:'Portfolio Build'    },
];
...
const WHY = [
  'Turnkey project delivery','Certified engineer workflow',
  'Tier-1 equipment catalog',  'Utility net-metering support',
  'Sample warranty terms','Demo technical support flow',
];
        ...
        <nav style={{ maxWidth:1200, margin:'0 auto', padding:'0 clamp(0.75rem, 4vw, 1.5rem)' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:10, minHeight:64 }}>
            <a href="/" style={{ display:'flex', alignItems:'center', gap:10, textDecoration:'none', flexShrink:0 }} onClick={scrollLock}>
              <img src={logo} alt={DEMO_COMPANY.name} style={{ width:36, height:36, borderRadius:10, objectFit:'cover', border:`2px solid ${ORANGE}` }}/>
              <span style={{ fontWeight:900, fontSize:'clamp(0.95rem, 4vw, 1.05rem)', color:BLACK, letterSpacing:'-.02em' }}>{DEMO_COMPANY.name}</span>
            </a>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'clamp(3rem, 10vw, 6rem) clamp(1rem, 4vw, 1.5rem)', width:'100%', position:'relative' }}>
          <div className="anim-fade-up" style={{ maxWidth:680 }}>
            ...
            <div className="anim-fade-up-3" style={{ display:'flex', flexWrap:'wrap', gap:'1.5rem', marginTop:'3rem', paddingTop:'2rem', borderTop:'1px solid rgba(255,255,255,.1)' }}>
              {[{icon:Star,label:'Sample Reviews'},{icon:CheckCircle,label:'Demo Workflow'},{icon:Shield,label:'Portfolio Build'},{icon:Zap,label:'Tier-1 Catalog'}].map(({icon:Icon,label})=>(
      ...
      <section id="gallery" style={{ background:WHITE, padding:'5rem 0' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 1.5rem' }}>
          <div style={{ textAlign:'center', marginBottom:'3rem' }}>
            ...
            <p style={{ color:'#9ca3af' }}>{DEMO_COMPANY.galleryNote}</p>
          </div>
          <div className="home-gallery-grid">
            ...
            <div style={{ gridColumn:'1/3', gridRow:'2/3', position:'relative', overflow:'hidden', borderRadius:16 }}>
              ...
              <div style={{ position:'absolute', bottom:18, left:20 }}>
                <span style={{ display:'inline-block', padding:'4px 12px', borderRadius:99, background:'rgba(247,148,29,.9)', color:WHITE, fontSize:'.72rem', fontWeight:800 }}>Sample Installations</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="products" style={{ background:'#f5f6f8', padding:'6rem 0' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 1.5rem' }}>
          <div style={{ textAlign:'center', marginBottom:'3.5rem' }}>
            ...
            <p style={{ color:'#9ca3af' }}>Sample product catalog for the portfolio demo.</p>
          </div>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 clamp(1rem, 4vw, 1.5rem)', paddingBottom:'clamp(2rem, 8vw, 4rem)' }} className="home-why-grid">
          <div style={{ position:'relative' }}>
            ...
            <div style={{ position:'absolute', top:24, left:24, padding:'10px 18px', borderRadius:14, background:ORANGE, boxShadow:'0 8px 24px rgba(247,148,29,.4)' }}>
              <p style={{ fontWeight:900, fontSize:'1.5rem', color:WHITE, letterSpacing:'-.03em', lineHeight:1 }}>120+</p>
              <p style={{ fontSize:'.72rem', color:'rgba(255,255,255,.8)', fontWeight:600, marginTop:2 }}>Sample Projects</p>
            </div>
          </div>

          <div style={{ paddingBottom:32 }}>
            <span style={{ display:'inline-block', padding:'5px 14px', borderRadius:99, background:'rgba(247,148,29,.1)', border:`1px solid rgba(247,148,29,.25)`, color:ORANGE, fontSize:'.72rem', fontWeight:800, textTransform:'uppercase', letterSpacing:'.1em', marginBottom:'1.25rem' }}>Why {DEMO_COMPANY.name}</span>
            <h2 style={{ fontSize:'clamp(1.8rem,3.5vw,2.75rem)', fontWeight:900, color:BLACK, letterSpacing:'-.03em', marginBottom:'1rem' }}>
              A Sample Solar Brand<br/>for This Demo
            </h2>
            <p style={{ color:'#6b7280', lineHeight:1.75, marginBottom:'2rem', maxWidth:420 }}>
              {DEMO_COMPANY.whyBody}
            </p>
      ...
      <section id="contact" style={{ background:BLACK, padding:'6rem 0' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 1.5rem' }}>
          <div style={{ textAlign:'center', marginBottom:'3.5rem' }}>
            ...
            <p style={{ color:'rgba(255,255,255,.35)', marginTop:12, fontSize:'.9rem' }}>Fictional contact details for this portfolio demo</p>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))', gap:'1.25rem' }}>
            {[
              { icon:Phone,  title:'Call Us',  lines:DEMO_COMPANY.phones },
              { icon:Mail,   title:'Email Us', lines:[DEMO_COMPANY.email] },
              { icon:MapPin, title:'Visit Us', lines:DEMO_COMPANY.addressLines },
      ...
      <footer style={{ background:'#0a0a0a', borderTop:'1px solid rgba(255,255,255,.06)', padding:'1.75rem 0' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 1.5rem', display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:'1rem' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <img src={logo} alt={DEMO_COMPANY.name} style={{ width:30, height:30, borderRadius:8, objectFit:'cover', border:`1.5px solid ${ORANGE}` }}/>
            <span style={{ fontWeight:900, color:'rgba(255,255,255,.4)', fontSize:'.875rem' }}>{DEMO_COMPANY.name}</span>
          </div>
          <p style={{ fontSize:'.78rem', color:'rgba(255,255,255,.2)' }}>
            © {new Date().getFullYear()} {DEMO_COMPANY.name} — Portfolio demo (fictional company data)
          </p>
